    ...
    private async Task CompleteSetStatus()
    {
        switch (settingStatus)
        {
            case "completed":
                ...
                suggestion.OwnerNotes = $"You are right, this is an important topic for devs. We created a resourse about it here: <a href='{urlText}' target='_blank'>{urlText}</a>";
                break;
            case "watching":
                suggestion.SuggestionStatus = statuses.Where(s => s.StatusName.ToLower() == settingStatus.ToLower()).First();
                suggestion.OwnerNotes = $"We noticed the interest, and if more people are interested we may address the topic.";
                break;
            case "upcoming":
                suggestion.SuggestionStatus = statuses.Where(s => s.StatusName.ToLower() == settingStatus.ToLower()).First();
                suggestion.OwnerNotes = $"We have a resource in the pipeline to address this topic.";
                break;
            case "dismissed":
                suggestion.SuggestionStatus = statuses.Where(s => s.StatusName.ToLower() == settingStatus.ToLower()).First();
                suggestion.OwnerNotes = $"This doesn't fit within the scope of our requirements.";
                break;
            default:
                return;
        }
